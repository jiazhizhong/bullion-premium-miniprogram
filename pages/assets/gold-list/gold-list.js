// pages/assets/gold-list/gold-list.js
const app = getApp();
...
Page({
  ...
  async loadAssets() {
    ...
    try {
      // 从API获取资产列表（带搜索条件）
      const assetsData = await api.getAssets('gold', this.data.searchDate || undefined, this.data.searchChannel || undefined);
      if (assetsData && assetsData.gold) {
        const assets = assetsData.gold.map(item => ({
          ...item,
        ...
        // 更新全局数据
        app.globalData.assets.gold = assetsData.gold;
      } else {
        // 失败时使用全局数据并应用筛选
  ...
  // 本地筛选资产（当接口失败时使用）
  filterLocalAssets() {
    let assets = app.globalData.assets.gold || [];
